					...
					do {countRuns++;
						...
						// try {
							do {countIterations++;
								...
								(run.iterations.length < MIN_ITERS) // Must continue if it hasn't done all its chores
								||
								(iteration.time < MIN_ITER_TIME) // Must continue if it hasn't completed a full days work
							)
						// }catch(err){
						// 	run.end = new Date();
						// 	throw(err);
						...
						||
						(run.time < MIN_RUN_TIME) // Must continue if it hasn't completed a full days work
					);
