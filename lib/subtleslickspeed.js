...
var SubtleSlickSpeed = parent.SubtleSlickSpeed || (function(){
	...
	Runner.prototype = {
		...
		run: function(){
			...
			tests = tests.sort(Array.makeSortByKey('id'));
			
			var runNextTest = function runNextTest(){
				if (tests.length) tests.pop().run();
	...
	Test.prototype = Object.merge(
		Events,
		{
					...
					do {this.data.runs.count++;
						...
						// try {
							do {this.data.iterations.count++;
								...
								iteration.result = this.fn.call(this.binding, this.data.iterations.count);
			...
			getData: function(){
				...
				dit.times = dit.times.sort(function(a,b){return a-b;});
