...
SubtleSlickSpeed.Report.prototype.buildOne = function(test){
	if (!this.table.hasSetCells) {
		...
		for (var frameworkName in Frameworks) {
			this.table.set('header', frameworkName, 'loading… 0');
			for (var i=0; i < SELECTORS.length; i++) {
				if (SELECTORS[i]) this.table.set(SELECTORS[i], 'header', 'loading… 1');
				if (SELECTORS[i]) this.table.set(SELECTORS[i], frameworkName, 'loading… 2');
			}
		}
