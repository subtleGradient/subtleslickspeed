	...
	this.table.set('header', d.framework, function(){
		...
		html.push(Build.tag('div', { 'class':'bgw' },
			function(){
				...
				html.push(
					Build.tag('div', { 'class':'bg '+className, style:'width:' + n + '%;height:' + n + '%;font-size:' + n + '%;opacity:' + n/100}, (Math.round(n1/100 *10)/10) +'x' )
				);
		...
		html.push(Build.tag('div', { 'class':'bgw' },
			function(){
				...
				for (var i=0; i < a.length; i++) {
					...
					html.push(
						Build.tag('div', { 'class':'bg '+a[i][a[i].length-1], style:'width:' + n + '%;height:' + n + '%;font-size:' + n + '%;opacity:' + n/100}, (Math.round(n1/100 *10)/10) +'x' )
					);
				}
