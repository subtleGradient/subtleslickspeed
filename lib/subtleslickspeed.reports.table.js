...
try{
	...
	Number.setCompare(d.selector, d.iterations.time.average);
	Number.setCompare(d.framework, d.iterations.time.average);
	Number.setCompare('All', d.iterations.time.average);
		...
		html.push(Build.tag('div', { 'class':'bgw' },
			function(){
				...
				for (var i=0; i < a.length; i++) {
					n = Number.getCompare(a[i][0], d.iterations.time.average).percent.max * 100;
					html.push(
						Build.tag('div', { 'class':'bg '+a[i][a[i].length-1], style:'width:' + n + '%'}, (Math.round(n *10)/10) +'%' )
					);
				}
