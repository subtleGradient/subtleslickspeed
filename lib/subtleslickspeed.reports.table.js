	...
	Table.prototype = {
		...
		rebuild: function(){
			for (var row_id in this.rows) {
				var cells = this.rows[row_id];
				var cell_ids = [];
				for (var cell_id in cells) {
					cell_ids.push(cell_id);
				}
				var first_cell = cell_ids[0];
				delete cell_ids[0];
				cell_ids = cell_ids.sort();
				cell_ids.pop();
				cell_ids[0] = first_cell;
				
				
				var new_cells = [];
				for (var i=0; i < cell_ids.length; i++) {
					new_cells.push(cells[cell_ids[i]]);
				}
				this.rows[row_id] = new_cells;
			}
