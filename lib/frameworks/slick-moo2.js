...
(function(){
	...
	function getByXPATH(context, XPATH_Selector){
		var THEM = [], item;
		var xpathResult = document.evaluate(XPATH_Selector, context, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		for (var i = 0, length = xpathResult.snapshotLength; i < length; i++)
			THEM.push(xpathResult.snapshotItem(i));
		return THEM;
	}
	...
	var combinators = {
		...
		'>>': function(node, tag, id, parts){ // all child nodes, any level
			...
			if ((node.getElementsByClassName /*|| typeof XPathResult != 'undefined'*/) && parts.length == 1 && parts[0].type == 'class'){
				// FIXME: Support multiple classNames!
				
				// if (node.getElementsByClassName)
					children = node.getElementsByClassName(parts[0].value);
				
				// if (typeof XPathResult != 'undefined')
				// 	children = getByXPATH(node, '//*[contains(concat(" ", @class, " "), " '+parts[0].value+' ")]');
